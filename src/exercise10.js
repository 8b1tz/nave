...
Faça uma função que receba 2 parâmetros: 
um array de movies e um array de actors.
 A função deve retornar um array de movies, 
 onde cada movie possui a propriedade actors, 
...
window.exercise10 = function () {
  ...
  function teste(movies, actors) {
    for (let m of movies) {
      m.actors = [];
      for (let a of actors) {
        for (let i = 0; i < 3; i++) {
          if (a.movie_ids[i] === m.id) {
            m.actors.push(a.name);
          }
        }
      }
    }
  ...
  teste(movies, actors);
  movies.forEach((p) => console.log(p));
};
